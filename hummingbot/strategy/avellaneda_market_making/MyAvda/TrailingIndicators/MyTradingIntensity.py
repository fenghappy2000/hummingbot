...
class MyTradingIntensityIndicator:
    ...
    @last_quotes.setter
    def last_quotes(self, value) -> None:
        """A helper method to be used in unit tests"""
        self._last_quotes = value

    def calculate(self, ts: float) -> None:
        """A helper method to be used in unit tests"""
        self.c_calculate(ts)

    def c_calculate(self, timestamp) -> None:
    ...
    def register_trade(self, trade: object) -> None:
        """A helper method to be used in unit tests"""
        self.c_register_trade(trade)

    def c_register_trade(self, trade: object) -> None:
        self._current_trade_sample.append(trade)

    def c_estimate_intensity(self) -> None:
        # cdef:
        ...
        lambdas_adj: List[float] = [10**-10 if x == 0 else x for x in lambdas]
